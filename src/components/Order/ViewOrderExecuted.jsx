...
const ViewOrderExecuted = () => {
  ...
  const PRODUCTS_PER_PAGE = 50; // Load 50 products at a time
  ...
  useEffect(() => {
    ...
    let filteredProducts = productId;
    if (productSearchTerm) {
      const searchLower = productSearchTerm.toLowerCase();
      filteredProducts = productId.filter(prod => 
        prod.productId && prod.productId.toLowerCase().includes(searchLower)
      );
    }
  ...
  }, [productId, productSearchTerm, productPage]);
  ...
  const handleMenuScroll = (event) => {
    ...
    // If scrolled to bottom and not all products loaded
  ...
  const getProductOptions = useCallback(() => {
    // If no products loaded yet, show loading state
    if (!productId || productId.length === 0) {
      return [{ label: 'Loading products...', value: null, isDisabled: true }];
    }

    // If search term exists and no results
    ...
    return options;
  }, [displayedProducts, allProductsLoaded, productLoading, productId, productSearchTerm]);
  ...
  const getOrder = async (page, filters = {}) => {
    try {
      const response = await fetch(
        `${VIEW_ALLACCEPTED_ORDERS}?page=${page || 1}`,
        {
          method: 'POST',
          headers: {
        ...
        <td className="px-5 py-5 border-b border-gray-200 text-sm">
          <p className="flex text-gray-900 whitespace-no-wrap">
            <FiEdit
              ...
              onClick={() => navigate(`/Order/updateorderExecuted/${item?.id}`)}
              title="Edit Order"
            />
            <FiTrash2
              size={17}
  ...
  // Custom menu list component with scroll handling
                    ...
                    <div className="flex-1 min-w-[300px]">
                      ...
                      <div className="z-20 bg-transparent dark:bg-form-Field">
                        <ReactSelect
                          ...
                          options={[
                            { label: 'View All Suppliers', value: '' },
                            ...formattedSupplier,
                          ]}
                          styles={customStyles}
                  ...
                  <div className="mb-4.5 flex flex-wrap gap-6 mt-12">
                    ...
                    <div className="flex-1 min-w-[200px]">
                      ...
                      <ReactSelect
                        ...
                        onBlur={handleBlur}
                        options={[
                          { label: 'View All Customers', value: '' },
                          ...formattedCustomer,
                        ]}
                        styles={customStyles}
                        className="bg-white dark:bg-form-input"
