...
const ViewOrderPending = () => {
  ...
  // Helper function to safely get product display
  ...
  const renderTableRows = () => {
    if (!Order || !Order.length) {
      return (
        <tr className='bg-white dark:bg-slate-700 dark:text-white'>
          <td colSpan="7" className="px-5 py-5 border-b border-gray-200 text-sm">
    ...
    return Order.map((item, index) => (
      <tr key={item.id || index} className='bg-white dark:bg-slate-700 dark:text-white'>
        ...
        <td className="px-5 py-5 border-b border-gray-200 text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{getProductDisplay(item?.productId)}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{item?.challanNo || '-'}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{formatDate(item?.challanDate)}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{formatDate(item?.expectedDate)}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 text-sm">
          <p className="flex text-gray-900 whitespace-no-wrap">
            <FiEdit 
              ...
              onClick={() => navigate(`/Order/updateChallan/${item?.id}`)} 
              title='Edit Order' 
            />
            <FiTrash2 
              size={17} 
