...
import { GET_CONT_SAREE_URL, UPDATE_ORDER_URL, UPDATE_ISSUECHALLAN } from "../../Constants/utils";
import { useNavigate } from "react-router-dom";

const SareeSuppliers = () => {
  ...
  const handleViewSupplier = (supplierId) => {
    if (supplierId) {
      navigate(`/supplierSaree-order/${supplierId}`);
    } else {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      ...
      const apiUrl = `${GET_CONT_SAREE_URL}`;
  ...
  const fetchPageFromServer = async (pageNumber) => {
    try {
      ...
      const apiUrl = `${GET_CONT_SAREE_URL}?page=${pageIndex}&size=${pagination.itemsPerPage}`;
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:bg-boxdark mb-6">
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/Saree-in-progress")}
