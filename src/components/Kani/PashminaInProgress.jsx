...
import { DELETE_ORDER_URL, VIEW_APPROVED_PASHMINAEMBROIDERY_ORDERS } from "../../Constants/utils"; // Changed here
...
const PashminaInProgress = () => {
   ...
   const getOrder = async (page, filters = {}) => {
    console.log("Fetching Pashmina Embroidery orders with filters:", filters); // Updated log
    ...
    try {
        ...
        const url = `${VIEW_APPROVED_PASHMINAEMBROIDERY_ORDERS}?${queryParams}`; // Changed here
        ...
        console.log("Parsed Pashmina Embroidery orders data:", data); // Updated log

        if (data?.content && data.content.length > 0) {
            setOrder(data.content);
        } else {
            console.log("No Pashmina Embroidery orders found on this page"); // Updated log
            setOrder([]);
        }
        ...
        console.error("Error fetching Pashmina Embroidery orders:", error); // Updated log
        toast.error("Failed to fetch Pashmina Embroidery orders. Please try again."); // Updated message
        setOrder([]);
    }
};
            ...
            return (
                <tr className='bg-white dark:bg-slate-700 dark:text-white'>
                    <td colSpan="6" className="px-5 py-5 border-b border-gray-200 text-sm">
                        <p className="text-gray-900 whitespace-no-wrap text-center">No Pashmina Embroidery Order Found</p> {/* Updated message */}
                    </td>
                </tr>
            );
        }
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName="Pashmina In Progress Orders" /> {/* Updated page name */}
            <div className="container mx-auto px-4 sm:px-8 bg-white dark:bg-slate-800">
                <div className="pt-5">
                    <div className='flex justify-between'>
                        <h2 className="text-xl font-semibold leading-tight">PASHMINA EMBROIDERY IN PROGRESS ORDERS</h2> {/* Updated title */}
                    </div>
...
export default PashminaInProgress