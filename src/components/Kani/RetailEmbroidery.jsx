...
import { GET_RETAIL_EMBROIDERY_URL, GET_IMAGE } from "../../Constants/utils"; // Changed here
import { FiEdit } from "react-icons/fi";
import { useNavigate } from 'react-router-dom';

const RetailEmbroidery = () => {
  ...
  useEffect(() => {
    fetchRetailOrders(); // Renamed function for clarity
  ...
  // Fetch retail embroidery orders with proper pagination - RENAMED FUNCTION
  const fetchRetailOrders = async () => {
    console.log("Fetching Retail Embroidery orders for page:", pagination.currentPage, "size:", pagination.itemsPerPage); // Updated log
    ...
    try {
      ...
      apiUrl = `${GET_RETAIL_EMBROIDERY_URL}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`; // Changed here
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          await fetchRetailOrdersAlternative(); // Updated function name
          return;
        }
        
        throw new Error(`Failed to fetch Retail Embroidery Orders: ${response.status} - ${responseText}`); // Updated error message
      }
      ...
      console.error("Error fetching Retail Embroidery orders:", err); // Updated log
      setError(err.message);
      toast.error(err.message || "Failed to load Retail Embroidery orders"); // Updated message
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchRetailOrdersAlternative = async () => {
    try {
      ...
      apiUrl = `${GET_RETAIL_EMBROIDERY_URL}`; // Changed here
      ...
      apiUrl = `${GET_RETAIL_EMBROIDERY_URL}?size=${pagination.itemsPerPage}`; // Changed here
      ...
      apiUrl = `${GET_RETAIL_EMBROIDERY_URL}?page=${pageOneBased}&size=${pagination.itemsPerPage}`; // Changed here
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Retail Embroidery Orders" /> {/* Updated page name */}
        ...
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          <div>
            <h3 className="text-xl font-medium text-slate-500 dark:text-white">
              Retail Embroidery Orders {/* Updated title */}
            </h3>
...
export default RetailEmbroidery;