...
import { GET_COTTON_URL, UPDATE_ORDER_URL, UPDATE_ISSUECHALLAN } from "../../Constants/utils";
import { useNavigate } from "react-router-dom";

const CottonSuppliers = () => {
  ...
  const handleViewSupplier = (supplierId) => {
    if (supplierId) {
      navigate(`/cottonSuppOrd/${supplierId}`);
    } else {
  ...
  const fetchPageFromServer = async (pageNumber) => {
    try {
      ...
      const apiUrl = `${GET_COTTON_URL}?page=${pageIndex}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Cotton Suppliers" />
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:bg-boxdark mb-6">
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/cotton-in-progress")}
...
export default CottonSuppliers;