...
import { GET_CONTEMPORARY_URL, UPDATE_ORDER_URL, UPDATE_ISSUECHALLAN } from "../../Constants/utils";
import { useNavigate } from "react-router-dom";

const ContemporarySuppliers = () => {
  ...
  const handleViewSupplier = (supplierId) => {
    if (supplierId) {
      navigate(`/contemporarySuppOrd/${supplierId}`);
    } else {
  ...
  const fetchContemporaryOrders = async () => {
    ...
    try {
      console.log("Fetching Contemporary orders...");
      
      // First try without pagination to get all data
      const apiUrl = `${GET_CONTEMPORARY_URL}`;
  ...
  const fetchPageFromServer = async (pageNumber) => {
    try {
      ...
      const apiUrl = `${GET_CONTEMPORARY_URL}?page=${pageIndex}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Contemporary Suppliers" />
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:bg-boxdark mb-6">
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/contemporary-in-progress")}
...
export default ContemporarySuppliers;