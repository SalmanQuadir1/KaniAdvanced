...
import { DELETE_ORDER_URL, GET_COTTON_INPROGRESS_URL } from "../../Constants/utils";
...
const CottonInProgress = () => {
   ...
   const getOrder = async (page, filters = {}) => {
    console.log("Fetching Cotton orders with filters:", filters);
    ...
    try {
        ...
        const url = `${GET_COTTON_INPROGRESS_URL}?${queryParams}`;
        ...
        console.log("Parsed Cotton orders data:", data);

        if (data?.content && data.content.length > 0) {
            setOrder(data.content);
        } else {
            console.log("No Cotton orders found on this page");
            setOrder([]);
        }
            ...
            return (
                <tr className='bg-white dark:bg-slate-700 dark:text-white'>
                    <td colSpan="6" className="px-5 py-5 border-b border-gray-200 text-sm">
                        <p className="text-gray-900 whitespace-no-wrap text-center">No Cotton Order Found</p>
                    </td>
                </tr>
            );
        }
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName="Cotton In Progress Orders" />
            <div className="container mx-auto px-4 sm:px-8 bg-white dark:bg-slate-800">
                <div className="pt-5">
                    <div className='flex justify-between'>
                        <h2 className="text-xl font-semibold leading-tight">COTTON IN PROGRESS ORDERS</h2>
                    </div>
...
export default CottonInProgress