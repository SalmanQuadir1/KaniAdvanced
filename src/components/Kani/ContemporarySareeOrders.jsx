...
import { GET_CONT_SAREE_URL, GET_IMAGE } from "../../Constants/utils";
import { FiEdit } from "react-icons/fi";
import { useNavigate } from 'react-router-dom';

const ContemporarySareeOrders = () => {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      ...
      apiUrl = `${GET_CONT_SAREE_URL}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`;
  ...
  const fetchKaniOrdersAlternative = async () => {
    try {
      ...
      apiUrl = `${GET_CONT_SAREE_URL}?size=${pagination.itemsPerPage}`;
      ...
      apiUrl = `${GET_CONT_SAREE_URL}?page=${pageOneBased}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Contemporary Saree Orders" />
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:bg-boxdark mb-6">
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/saree-suppliers")}
...
export default ContemporarySareeOrders;