...
import { GET_KLC_EMBROIDERY_URL, GET_IMAGE } from "../../Constants/utils"; // Changed here
...
const KlcEmbroidery = () => {
  ...
  useEffect(() => {
    fetchKlcOrders(); // Renamed function
  ...
  // Fetch KLC embroidery orders with proper pagination - RENAMED FUNCTION
  const fetchKlcOrders = async () => {
    console.log("Fetching KLC Embroidery orders for page:", pagination.currentPage, "size:", pagination.itemsPerPage); // Updated log
    ...
    try {
      ...
      apiUrl = `${GET_KLC_EMBROIDERY_URL}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`; // Changed here
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          await fetchKlcOrdersAlternative(); // Updated function name
          return;
        }
        
        throw new Error(`Failed to fetch KLC Embroidery Orders: ${response.status} - ${responseText}`); // Updated error message
      }
      ...
      console.error("Error fetching KLC Embroidery orders:", err); // Updated log
      setError(err.message);
      toast.error(err.message || "Failed to load KLC Embroidery orders"); // Updated message
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchKlcOrdersAlternative = async () => {
    try {
      ...
      apiUrl = `${GET_KLC_EMBROIDERY_URL}`; // Changed here
      ...
      apiUrl = `${GET_KLC_EMBROIDERY_URL}?size=${pagination.itemsPerPage}`; // Changed here
      ...
      apiUrl = `${GET_KLC_EMBROIDERY_URL}?page=${pageOneBased}&size=${pagination.itemsPerPage}`; // Changed here
  ...
  // Render product details for each order product in the same row - FIXED: Always returns array
  const renderOrderProducts = (order, orderIndex) => {
    if (!order.products || order.products.length === 0) {
      // Return an array with a single element, not a single element
      return [
  ...
  // FIXED: renderTableRows with proper array handling
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="KLC Embroidery Orders" /> {/* Updated page name */}
        ...
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          <div>
            <h3 className="text-xl font-medium text-slate-500 dark:text-white">
                KLC Embroidery Orders {/* Updated title */}
            </h3>
