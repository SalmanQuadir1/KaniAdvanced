...
import { GET_RETAIL_PAPIERMACGE_URL, GET_IMAGE } from "../../Constants/utils";
...
const RetailPapierMache = () => {
  ...
  // Fetch Retail Papier Mache orders with proper pagination
  const fetchRetailPapierMacheOrders = async () => {
    console.log("Fetching Retail Papier Mache orders for page:", pagination.currentPage, "size:", pagination.itemsPerPage);
    ...
    try {
      ...
      apiUrl = `${GET_RETAIL_PAPIERMACGE_URL}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`;
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          await fetchRetailPapierMacheOrdersAlternative();
          return;
        }
        
        throw new Error(`Failed to fetch Retail Papier Mache Orders: ${response.status} - ${responseText}`);
      }
      ...
      console.error("Error fetching Retail Papier Mache orders:", err);
      setError(err.message);
      toast.error(err.message || "Failed to load Retail Papier Mache orders");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchRetailPapierMacheOrdersAlternative = async () => {
    try {
      ...
      apiUrl = `${GET_RETAIL_PAPIERMACGE_URL}`;
      ...
      apiUrl = `${GET_RETAIL_PAPIERMACGE_URL}?size=${pagination.itemsPerPage}`;
      ...
      apiUrl = `${GET_RETAIL_PAPIERMACGE_URL}?page=${pageOneBased}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Retail Papier Mache Orders" />
