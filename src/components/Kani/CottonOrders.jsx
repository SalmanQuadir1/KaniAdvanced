...
import { GET_COTTON_URL, GET_IMAGE } from "../../Constants/utils";
...
const CottonOrders = () => {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      ...
      apiUrl = `${GET_COTTON_URL}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`;
  ...
  const fetchKaniOrdersAlternative = async () => {
    try {
      ...
      apiUrl = `${GET_COTTON_URL}?size=${pagination.itemsPerPage}`;
      ...
      apiUrl = `${GET_COTTON_URL}?page=${pageOneBased}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Cotton Orders" />
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:bg-boxdark mb-6">
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/Cotton_filter-suppliers")}
