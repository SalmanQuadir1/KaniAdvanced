...
import { GET_WOOL_ORDERS_URL, UPDATE_ORDER_URL, UPDATE_ISSUECHALLAN } from "../../Constants/utils";
...
const WoolSuppliers = () => {
  ...
  const handleViewSupplier = (supplierId) => {
    if (supplierId) {
      navigate(`/woolSuppOrd/${supplierId}`);
    } else {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      ...
      const apiUrl = `${GET_WOOL_ORDERS_URL}`;
  ...
  const fetchPageFromServer = async (pageNumber) => {
    try {
      ...
      const apiUrl = `${GET_WOOL_ORDERS_URL}?page=${pageIndex}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName=" Wool Embroidery Suppliers" />
