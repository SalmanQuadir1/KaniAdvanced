...
import { GET_WOOL_SUPPORDERS_URL, GET_IMAGE } from "../../Constants/utils";
import { FiEdit } from "react-icons/fi";
import { useNavigate, useParams } from 'react-router-dom';

const WoolSupplierOrders = () => {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      ...
      const apiUrl = `${GET_WOOL_SUPPORDERS_URL}/${supplierId}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`;
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          const apiUrlWithoutPagination = `${GET_WOOL_SUPPORDERS_URL}/${supplierId}`;
...
export default WoolSupplierOrders;