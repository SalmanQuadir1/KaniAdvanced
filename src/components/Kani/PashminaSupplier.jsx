...
import { GET_PASHMINA_ORDERS_URL, GET_IMAGE } from "../../Constants/utils";
import { FiEdit } from "react-icons/fi";
import { useNavigate, useParams } from 'react-router-dom';

const PashminaSupplier = () => {
  ...
  // UPDATED: Fetch orders using GET_PASHMINA_ORDERS_URL
  const fetchKaniOrders = async () => {
    console.log("Fetching pashmina orders for supplier:", supplierId);
    ...
    try {
      // UPDATED: Using GET_PASHMINA_ORDERS_URL instead of GET_SUPPLIER_ORDERS_URL
      const apiUrl = `${GET_PASHMINA_ORDERS_URL}/${supplierId}?page=${pagination.currentPage}&size=${pagination.itemsPerPage}`;
      console.log("Fetching from updated URL:", apiUrl);
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          const apiUrlWithoutPagination = `${GET_PASHMINA_ORDERS_URL}/${supplierId}`;
      ...
      console.error("Error fetching pashmina orders:", err);
...
export default PashminaSupplier;