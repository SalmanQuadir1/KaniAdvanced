...
import { GET_CONTWOOL_ORDERS_URL, UPDATE_ORDER_URL, UPDATE_ISSUECHALLAN } from "../../Constants/utils";
import { useNavigate } from "react-router-dom";

const ContWoolSuppliers = () => {
  ...
  const handleViewSupplier = (supplierId) => {
    if (supplierId) {
      navigate(`/Cont_supplier-order/${supplierId}`);
    } else {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      ...
      const apiUrl = `${GET_CONTWOOL_ORDERS_URL}`;
  ...
  const fetchPageFromServer = async (pageNumber) => {
    try {
      ...
      const apiUrl = `${GET_CONTWOOL_ORDERS_URL}?page=${pageIndex}&size=${pagination.itemsPerPage}`;
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Contemporary Wool Suppliers" />
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:bg-boxdark mb-6">
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/contWool-in-progress")}
...
export default ContWoolSuppliers;