...
import { GET_PASHMINA_URL, UPDATE_ORDER_URL, UPDATE_ISSUECHALLAN } from "../../Constants/utils"; // Changed here
import { useNavigate } from "react-router-dom";

const PashminaSuppliers = () => {
  ...
  const fetchKaniOrders = async () => {
    ...
    try {
      console.log("Fetching Pashmina orders..."); // Updated log
      
      // First try without pagination to get all data - USING GET_PASHMINA_URL
      const apiUrl = `${GET_PASHMINA_URL}`; // Changed here
      ...
      toast.error(err.message || "Failed to load Pashmina orders"); // Updated message
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchPageFromServer = async (pageNumber) => {
    try {
      ...
      const apiUrl = `${GET_PASHMINA_URL}?page=${pageIndex}&size=${pagination.itemsPerPage}`; // Changed here
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Pashmina Suppliers" /> {/* Updated page name */}
        ...
        <div className="flex justify-between items-center border-b border-stroke dark:border-strokedark py-4 px-6">
          <div>
            <h3 className="text-xl font-medium text-slate-500 dark:text-white">
              Pashmina Suppliers {/* Updated title */}
            </h3>
          ...
          <div>
            <button
              type="button"
              onClick={() => navigate("/inProgress-pashminaEmbroidery")}
...
export default PashminaSuppliers;